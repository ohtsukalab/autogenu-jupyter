...
def generateCpp(dimx, dimu, dimc, fxu, Cxu, phix, hx, hu):
    ...
    f_model_cpp.writelines([linecache.getline('codegen_modules/.templates_for_codeGen/nmpc_model.cpp', i) for i in range(0,11)])
    f_model_cpp.writelines(['    f[%d] = '%i + sympy.ccode(fxu[i]) + ';\n' for i in range(dimx)])
    f_model_cpp.writelines([linecache.getline('codegen_modules/.templates_for_codeGen/nmpc_model.cpp', i) for i in range(12,22)])
    f_model_cpp.writelines(['    phix[%d] = '%i + sympy.ccode(phix[i]) + ';\n' for i in range(dimx)])
    f_model_cpp.writelines([linecache.getline('codegen_modules/.templates_for_codeGen/nmpc_model.cpp', i) for i in range(23,36)])
    f_model_cpp.writelines(['    hx[%d] = '%i + sympy.ccode(hx[i]) + ';\n' for i in range(dimx)])
    f_model_cpp.writelines([linecache.getline('codegen_modules/.templates_for_codeGen/nmpc_model.cpp', i) for i in range(37,50)])
...
def generateHpp(dimx, dimu, dimc, scalar_params, array_params):
    ...
    f_model_hpp.writelines([linecache.getline('codegen_modules/.templates_for_codeGen/nmpc_model.hpp', i) for i in range(0,18)])
    ...
    f_model_hpp.writelines([linecache.getline('codegen_modules/.templates_for_codeGen/nmpc_model.hpp', i) for i in range(18,51)])
