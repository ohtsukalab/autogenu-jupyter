from AutoGenU import solver_params as solverparam
from AutoGenU import simulation_params as simparam
from AutoGenU import cpp_executor
import sympy
import linecache
import subprocess
import platform
...
def generateCpp(dimx, dimu, dimc, fxu, Cxu, phix, hx, hu, model_name):
    ...
    f_model_cpp.writelines([linecache.getline('AutoGenU/.cpp_templates/nmpc_model.cpp', i) for i in range(12,22)])
    f_model_cpp.writelines(['    phix[%d] = '%i + sympy.ccode(phix[i]) + ';\n' for i in range(dimx)])
    f_model_cpp.writelines([linecache.getline('AutoGenU/.cpp_templates/nmpc_model.cpp', i) for i in range(23,36)])
    f_model_cpp.writelines(['    hx[%d] = '%i + sympy.ccode(hx[i]) + ';\n' for i in range(dimx)])
    f_model_cpp.writelines([linecache.getline('AutoGenU/.cpp_templates/nmpc_model.cpp', i) for i in range(37,50)])
...
def generateCMake(solver_index, model_name):
    ...
    if(solver_index == 1):
        ...
        f_cmake.write(')\n')
        f_cmake.write('target_include_directories(\n')
        f_cmake.write('    cgmres\n')
        f_cmake.write('    PRIVATE\n')
        f_cmake.write('    ${MODEL_DIR}\n')
