...
void ContinuationGMRES::computeOptimalityError(const double time_param, const double* state_vec, const double* current_solution_vec, double* optimality_vec)
{
    ...
    tau = time_param;
    for(int i=0; i<horizon_division_num_; i++, tau+=delta_tau){
        model_.huFunc(tau, state_mat_[i], current_solution_vec+i*dim_control_input_and_constraints_, lambda_mat_[i+1], optimality_vec+i*dim_control_input_and_constraints_);
    }
}
...
void ContinuationGMRES::bFunc(const double time_param, const double* state_vec, const double* current_solution_vec, double* b_vec)
{
    ...
    for(int i=0; i<dim_solution_; i++){
        b_vec_[i] = (1/difference_increment_-zeta_)*optimality_vec_[i] - optimality_vec_2_[i]/difference_increment_;
    }
}
...
{
    deleteVector(dx_vec_);
    deleteVector(incremented_state_vec_);
    deleteVector(solution_vec_);
    deleteVector(incremented_solution_vec_);
    deleteVector(optimality_vec_);
    deleteVector(optimality_vec_1_);
    deleteVector(optimality_vec_2_);
    deleteVector(solution_update_vec_);
    deleteMatrix(state_mat_);
    deleteMatrix(lambda_mat_);
}
...
{
    double error_vec[dim_solution_] = {0};
