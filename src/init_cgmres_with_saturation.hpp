...
private:
    ...
    inline void computeSaturationOptimality(const Eigen::VectorXd& control_input_and_constraint_vec, const Eigen::VectorXd& lagrange_multiplier, const Eigen::VectorXd& saturation, Eigen::Ref<Eigen::VectorXd> optimality_vec);

    // Computes the optimality error vector under current_solution_vec.
    inline void computeOptimalityErrors(const double time_param, const Eigen::VectorXd& state_vec, const Eigen::VectorXd& control_input_and_constraint_vec, Eigen::Ref<Eigen::VectorXd> optimality_vec);
