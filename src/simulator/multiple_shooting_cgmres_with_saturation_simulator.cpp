...
void nmpcsim::simulation(MultipleShootingCGMRESWithSaturation& nmpc_solver, const double* initial_state_vec, const double start_time, const double end_time, const double sampling_period, const std::string save_dir, const std::string savefile_name)
{
    NMPCModel model;
    ...
    std::ofstream state_data(save_dir + "/" + savefile_name + "_state.dat");
    std::ofstream control_input_data(save_dir + "/" + savefile_name + "_control_input.dat");
    std::ofstream error_data(save_dir + "/" + savefile_name + "_error.dat");
