...
// phix = (q_terminal[0]*(x[0]-x_ref[0])^2 + q_terminal[1]*(x[1]-x_ref[1])^2 + q_terminal[2]*(x[2]-x_ref[2])^2 + q_terminal[3]*(x[3]-x_ref[3])^2)/2
...
//  partial derivative of the Hamiltonian H (H := L + \lambda^T f), dH/dx(t, x, u)
// L = (q[0]*(x[0]-x_ref[0])^2 + q[1]*(x[1]-x_ref[1])^2 + q[2]*(x[2]-x_ref[2])^2 + q[3]*(x[3]-x_ref[3])^2 + r[0]*u[0]^2)/2
